...
resource "azurerm_resource_group_template_deployment" "ztna_connector_1arm_autoscale" {
  ...
  template_content = file("${path.cwd}/../AZURE-PA-ZTNA-Connector-1ARM-Autoscale-auto1.json")
  ...
  parameters_content = jsonencode({
    ...
    "host2_ip" = {
      value = local.general_data.host2_ip
    }
    "host3_ip" = {
      value = local.general_data.host3_ip
    }
    "host4_ip" = {
      value = local.general_data.host4_ip
    }
    "host2_name" = {
      value = local.general_data.host2_name
    }
    "host3_name" = {
      value = local.general_data.host3_name
    }
    "host4_name" = {
      value = local.general_data.host4_name
    }
  })
