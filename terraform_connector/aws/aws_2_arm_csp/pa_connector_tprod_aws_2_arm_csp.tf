...
resource "aws_cloudformation_stack" "ztna_connector_2arm" {
  ...
  parameters = {
    ...
    PublicSubnet = local.subnet_data.external_subnet
    PrivateSubnet = local.subnet_data.internal_subnet
    CgnxHostName = local.instance_data.host1_name
    CgnxHostIp = local.instance_data.host1_ip
  }

  template_body = file("${path.cwd}/../AWS-PA-ZTNA-Connector-2ARM.yaml")
